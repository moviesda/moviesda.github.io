...
const AllGenresPage = ({ categories }: Props) => {
    return (
        <main className="mx-auto">
            <h1 className="text-3xl font-bold mb-8">All Director</h1>
            <div className="grid grid-cols-1 items-center justify-center md:grid-cols-4 gap-4">
                {categories.map((category) => (
                    <Link
                        key={category}
                        href={`/director/${encodeURIComponent(category.toLowerCase().replace(/\s+/g, '-'))}`}
...
export async function getStaticProps() {
    const categories = await getUniqueField('directors');
